...
Texture::Texture(const wchar_t* full_path) : Resource(full_path)
{
	...
	if (SUCCEEDED(res))
	{
		res = DirectX::CreateTexture(GraphicsEngine::get()->getRenderSystem()->getDirect3DDevice(), image_data.GetImages(),
		...
		GraphicsEngine::get()->getRenderSystem()->getDirect3DDevice()->CreateShaderResourceView(m_texture, &desc,
			&m_shader_res_view);
	}
	else
	{
